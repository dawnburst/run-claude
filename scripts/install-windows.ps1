...
# stderr therefore kills the script even when it succeeded. The case that
# caught this: running the installer with a UNC working directory, where
# cmd.exe prints "UNC paths are not supported" to stderr, so verifying the
# freshly installed shim aborted an install that had in fact worked.
...
$PyExe = $py.Source
$probe = Invoke-Capture $PyExe @('-c', "import sys; print('%d.%d' % sys.version_info[:2])")
try { $ver = [Version] $probe.Last } catch { Die "could not read the Python version (got '$($probe.Last)')." }
...
if (-not $Venv) {
    ...
    try {
        ...
        # and discards the result, so the process always exited 0 and every
        # exit code lmi defines was lost - fatal for an unattended tool.
        [System.IO.File]::WriteAllText(
            (Join-Path $stage '__main__.py'), $MainPy, [System.Text.Encoding]::UTF8)
        # The shebang is written for portability - Windows ignores it, and the
        # same file then also runs directly on Linux and macOS.
        $zip = Invoke-Capture $PyExe @('-m','zipapp',$stage,'-p','/usr/bin/env python3','-o',$built)
        if ($zip.Code -ne 0 -or -not (Test-Path $built)) { Die "zipapp failed to build the executable:`n    $($zip.Text)" }
        $check = Invoke-Capture $PyExe @($built, '--version')
        if ($check.Code -ne 0) { Die "the built executable does not run: $($check.Text)" }
        $builtVer = $check.Last
    ...
    $venvUsable = $false
    if (Test-Path $venvPy) {
        $venvUsable = ((Invoke-Capture $venvPy @('-c','')).Code -eq 0)
    }
    if ($venvUsable) {
        Ok "reusing $VenvDir"
    } else {
        if (Test-Path $VenvDir) { Warn "existing $VenvDir is not usable, rebuilding it"; Remove-Item -Recurse -Force $VenvDir }
        $mk = Invoke-Capture $PyExe @('-m','venv',$VenvDir)
        if ($mk.Code -ne 0) { Die "python -m venv failed:`n    $($mk.Text)`n    Drop -Venv to use the default mode, which needs no virtual environment." }
        Ok "created $VenvDir"
    }
    Step "Installing lmi into it"
    $pipArgs = @('-m','pip','install','--quiet','--upgrade')
    if ($Editable) { $pipArgs += '--editable' }
    $pip = Invoke-Capture $venvPy ($pipArgs + $Repo)
    if ($pip.Code -ne 0) {
...
# --- 4. verify -------------------------------------------------------------
Step "Verifying"
$verify = Invoke-Capture $Shim @('--version')
if ($verify.Code -ne 0) { Die "$Shim did not run:`n    $($verify.Text)" }
Ok $verify.Last
